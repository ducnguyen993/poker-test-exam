...
export const prepareRank = (hand: number[]): Rank => {
    ...
    return {
        ...
        ranks,
        rankDescription,
        rankDisplayString: mappingCardNumberToString(hand),
    };
};
 

export const mappingCardNumberToString = (hand: number[]): string => {
    const suits = [`♣︎`, `♦︎`, `♥︎`, `♠︎`];
    return hand
        .map(
            (item) =>
